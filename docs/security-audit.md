...
**Status:** In progress — Phase 1 and ingress hardening updates complete 2026-02-17.
...
| S7 | No WebSocket origin validation | HIGH | **Fixed** | Origin allowlist validation at handshake (`ALLOWED_WS_ORIGINS`, `game-server.ts`) |

### Category 2: Gameplay Exploits

| ID | Finding | Severity | Status | Notes |
|----|---------|----------|--------|-------|
| G1 | No input rate limiting | HIGH | **Fixed** | Token-bucket limiter: 100 msg/sec max, constant-memory ingress guard (`game-server.ts`) |
...
| D2 | No connection rate limiting | MEDIUM | **Fixed** | Per-source handshake token bucket in `game-server.ts` (20 connections / 10s) |
| D3 | No per-player message rate limit | HIGH | **Fixed** | Same as G1 |
| D4 | MAX_PLAYERS not enforced on accept | MEDIUM | **Fixed** | `addPlayer()` checks `players.size >= MAX_PLAYERS`; rejected sockets are not tracked in server maps |
...
| `packages/server/src/__tests__/security/dos-resilience.test.ts` | S1, S4, S7, D1–D5, G1, G5 | 9 active |
