...
This table maps manual sections to test files. The tests are organized by game system rather than manual structure for developer clarity.

| Manual Section | Test File(s) | Notes |
|----------------|--------------|-------|
| Overview | _(no tests)_ | Introductory text only |
| **Tank Movement** | `02-tank-movement.test.ts` | Full coverage: terrain speed, acceleration, turning, collision, deep sea death |
| **Tank Spawning** | `01-tank-spawning.test.ts` | Initial state and resources |
| **Shooting** | `03-shooting.test.ts` | Shell basics, collisions (terrain/tank/pillbox/base), range, reload |
| **Terrain Damage** | `04-terrain-damage.test.ts` | Direct hits and explosion damage; crater flooding not yet implemented |
| **Mines** | `05-mines.test.ts` | Quick mines, builder-laid mines, explosions; chain reactions not yet implemented |
| **Boats** | `06-boats.test.ts` | Speed, boarding/disembarking, vulnerability, building |
| **Water Mechanics** | `07-water-mechanics.test.ts` | Resource drain mechanic (all skipped - not yet implemented) |
| **Pillboxes** | `08-pillboxes.test.ts` | Targeting, variable fire rate, damage; pickup/placement/repair/concealment not yet implemented |
| **Refueling Bases** | `09-bases.test.ts` | Refueling, capture; self-replenishment not yet implemented |
| Farming and Building | `10-builder.test.ts` | Lifecycle, harvesting, roads, walls, boats, mine laying; vulnerability/regrowth not yet implemented |
| **Tank Death/Respawn** | `11-death-respawn.test.ts` | Full coverage |
| **Alliances** | `12-alliances.test.ts` | Basic team mechanics only; alliance system not yet implemented |
| Building a Fortress | _(no tests)_ | Strategic guide, not testable mechanics |
| Mine Laying | `05-mines.test.ts` | Covered under Mines section |
| Mine Clearing | `03-shooting.test.ts` (range adjustment) | Manual range control for mine clearing |
| The Screen | _(no tests)_ | UI reference, not server-side mechanics |
| Remote Views | _(no tests)_ | UI feature, not server-side mechanics |
| Notes | _(no tests)_ | Gameplay tips, not testable mechanics |
| Other Maps | _(no tests)_ | Map loading covered in integration tests |
| Brains | _(no tests)_ | AI system, out of scope |
| **Win Condition** | `13-win-condition.test.ts` | Not yet implemented (all skipped) |

**Test File Summary:** 137 active tests, 48 skipped tests (unimplemented features)
